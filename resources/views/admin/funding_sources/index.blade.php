            ...
            <div class="col-lg-8">
                <div class="page-header-title">
                    ...
                    <div class="d-inline">
                        ...
                        <span>Listado de Tipos de Fuente de Financiamiento</span>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <div class="float-left">
                                        <h5>Listado de Fuentes de Financiamiento</h5>
                                    </div>
                                    <div class="float-right">
                                        <a href="{{ route('funding_sources.create') }}" class="btn btn-primary">Agregar Fuente de Financiamiento</a>  
                                    </div>
                                </div>
                                <div class="card-block">
                                    <div class="dt-responsive table-responsive">
                                        <table id="funding_sources" class="table table-striped table-bordered nowrap">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>Código</th>
                                            ...
                                            <tbody>
                                            @for ($i = 0; $i < count($funding_sources); $i++)
                                                <tr>
                                                    <td>{{ ($i+1) }}</td>
                                                    <td>{{ $funding_sources[$i]->code }}</td>
                                                    <td>{{ $funding_sources[$i]->description }}</td>
                                                    <td>
                                                        <button type="button" title="Editar" class="btn btn-warning btn-icon" onclick="updateFundingSource({{ $funding_sources[$i]->id }})">
                                                            <i class="fa fa-pencil"></i>
                                                        </button>
                                                        <button type="button" title="Borrar" class="btn btn-danger btn-icon" onclick="deleteFundingSource({{ $funding_sources[$i]->id }})">
...
$(document).ready(function(){
    $('#funding_sources').DataTable();

    updateFundingSource = function(funding_source){
        location.href = '/funding_sources/'+funding_source+'/edit/';
    }

    deleteFundingSource = function(funding_source){
      swal({
            title: "Atención",
        ...
        function(isConfirm){
          if(isConfirm){
            $.ajax({
              url : '/funding_sources/'+funding_source,
              method : 'POST',
