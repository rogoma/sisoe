...
class CreateQueriesResponsesTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('queries_responses', function (Blueprint $table) {
            $table->integer('id')->autoIncrement(); // set auto-increment and primary key
            $table->string('response', 300);
            $table->integer('query_id');	// se admite null                        
            ...
            // INDEXES
            $table->foreign('query_id')->references('id')->on('queries')->onUpdate('cascade');       
    ...
    {
        Schema::dropIfExists('queries_responses');
    }
}
