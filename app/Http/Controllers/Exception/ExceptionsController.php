...
class ExceptionsController extends Controller
{
    ...
    {
        $this->middleware('checkPermission:exceptions.orders.index,admin.orders.index')->only('index'); // Permiso para index 
        $this->middleware('checkPermission:exceptions.orders.show,admin.orders.show')->only('show'); // Permiso para show 
    }
    ...
    {
        // obtenemos los pedidos que sean Excepciones y que esén con estado mayor o igual a 20 (PROCESADO PLANIFICACIÓN)
        $orders = Order::wherein('modality_id', [9,10])
                        ->where('actual_state', '>=', 13)
                        ->get();
        return view('exception.exceptions.index', compact('orders'));
    }
    ...
    public function show(Request $request, $order_id)
    {
        ...
        $exception_dependency = $request->user()->dependency_id;
        // Obtenemos los simese cargados por otras dependencias
        $related_simese = $order->simese()->where('dependency_id', '!=', $exception_dependency)->get();
        // Obtenemos los simese cargados por excepciones
        $related_simese_exception = $order->simese()->where('dependency_id', $exception_dependency)->get();
        
        // Obtenemos los archivos cargados por otras dependencias y que no sean de reparo
        $other_files = $order->files()->where('dependency_id', '!=', $exception_dependency)
                                            ->whereIn('file_type', [0, 3, 4, 5])//0-antecedentes 3-contratos 4-addendas  5-dictamenes 
                                            ->orderBy('created_at','asc')
                                            ->get();

        // Obtenemos los archivos cargados por excepciones con tipo de archivos que no sean 1 (reparos dncp)
        $exception_files = $order->files()->where('dependency_id', $exception_dependency)->where('file_type', '=', 0)//Archivos normales                                                                                         
                                                                                         ->get();

        // Obtenemos los archivos cargados por licitaciones con tipo de archivos 7 cuadro comparativo
        $exception_files2 = $order->files()->where('dependency_id', $exception_dependency)->where('file_type', '=', 7)//cuadro comparativo
                                                                                         ->get(); 

        // Obtenemos los archivos cargados por excepciones con tipo de archivos que sean 1 (reparos dncp)
        $exception_filedncp = $order->files()->where('dependency_id', $exception_dependency)->where('file_type', '=', 1)->get();

        // Obtenemos los archivos cargados por excepciones con tipo de archivos que sean 6 (cosultas dncp)
        $exception_filedncp_con = $order->files()->where('dependency_id', $exception_dependency)->where('file_type', '=', 6)->get();
        ...
        // Obtenemos los archivos cargados por excepciones
        //$exception_files = $order->files()->where('dependency_id', $exception_dependency)->get();


        return view('exception.exceptions.show', compact('order', 'related_simese', 'queries',
                    'related_simese_exception', 'other_files', 'exception_files', 
                    'objections', 'exception_filedncp','exception_files2','exception_filedncp_con'));
    }
    ...
    public function recibeOrder(Request $request, $order_id){
        ...
        if(!$request->user()->hasPermission(['exceptions.orders.recibe'])){
            return response()->json(['status' => 'error', 'message' => 'No posee los suficientes permisos para realizar esta acción.'], 200);
        }

        $order = Order::find($order_id);
        // Estado 55 = RECIBIDO EXCEPCIONES
        $order->actual_state = 55;
        $order->save();
        ...
        $order_order_state->order_state_id = 55;
    ...
    public function edit(Request $request, $order_id)
    {
        ...
        if($request->user()->hasPermission(['exceptions.orders.update'])){
            return view('exception.orders.update', compact('order'));
        }else{
    ...
    public function update(Request $request, $order_id)
    {
        ...
        return redirect()->route('exceptions.show', $order_id)->with('success', 'Llamado modificado correctamente');;
    }
    ...
    public function deriveOrder(Request $request, $order_id){
        ...
        if(!$request->user()->hasPermission(['exceptions.orders.derive'])){
            return response()->json(['status' => 'error', 'message' => 'No posee los suficientes permisos para realizar esta acción.'], 200);
        }
    ...
    {
        // obtenemos los pedidos que sean de Compras Menores modality = 9 o 10
        // que estén con estado mayor o igual a 13 (DERIVADO DE DOC A PROC. COMPLEM. Y EXCEPCIONES PARA PROCESAR PEDIDO)
        // y estado menor o igual a 130 (DERIVADO DE DOC PARA PROCESAR PEDIDO)
        $orders = Order::whereIn('modality_id', [9,10])        
        ->where('actual_state', '>=', 13)->where('actual_state', '<=', 130)
        ->get(); 
