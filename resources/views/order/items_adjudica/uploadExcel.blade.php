                        ...
                        <div class="col-sm-12">
                            <div class="card">
                                <div class="card-header">
                                    ...
                                    <h5><p style="font-size: 17px; font-weight: bold; color:#FF0000">Agregar Ítem</p></h5>
                                    {{-- <h5><p style="font-size: 17px; font-weight: bold; color:#FF0000">Agregar Ítem al pedido Nº {{ $order->id }}</p></h5> --}}
                                        @if ($order->open_contract == 1)
                                            <h5 style="color:blue;background-color:yellow;font-weight: bold">   -   Adjuntar Archivo Excel de ítemes-Contrato Abierto</h5>                                            
                                        @else
                                            @if ($order->open_contract == 2)
                                                <h5 style="color:blue;background-color:yellow;font-weight: bold">   -  Adjuntar Archivo Excel de ítemes-Contrato Cerrado</h5>                                        
                                            @else
                                                <h5 style="color:blue;background-color:yellow;font-weight: bold">   -  Adjuntar Archivo Excel de ítemes-Contrato Abierto Mmin Mmáx</h5>
                                            @endif        
                                        @endif 
                                </div>
                                <div class="card-block">                                        
                                        <form method="POST" action="{{ route('orders.items_adjudica.storeExcelAw', $order->id) }}" enctype="multipart/form-data">

                                        {{-- Elejimos el POST de acuerdo al tipo de Contrato --}}
                                        {{-- @if ($order->open_contract == 1)
                                            <form method="POST" action="{{ route('orders.items_adjudica.storeExcel', $order->id) }}" enctype="multipart/form-data">
                                        @else
                                            @if ($order->open_contract == 2)
