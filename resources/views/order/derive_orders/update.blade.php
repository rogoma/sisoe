        ...
        <div class="row align-items-end">
            <div class="col-lg-8">
                <div class="page-header-title">
                    ...
                    <div class="d-inline">
                        <h5>Pedidos</h5>
                        <span>Modificar Procesar Pedido</span>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                                ...
                                <div class="card-block">
                                    <form id="formeEdit" method="POST">
                                            ...
                                            <div class="col-sm-6">
                                                <select id="verification" name="verification" class="form-control">
                                                    @foreach (array('ACEPTAR', 'RECHAZAR') as $row)
                                                        <option value="{{ $row }}" @if($row == $verification) selected @endif>{{ $row }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="col-sm-6">
                                                <select id="urgency_state" name="urgency_state" class="form-control">
                                                    @foreach (array('BAJA', 'MEDIA', 'ALTA') as $row)
                                                        <option value="{{ $row }}" @if($row==$order->urgency_state) selected @endif>{{ $row }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="form-group row @error('rejected_obs') has-danger @enderror">
                                            ...
                                            <div class="col-sm-10">                                                                                                
                                                <input type="text  " id="rejected_obs" name="rejected_obs" value="{{ old('rejected_obs',$order->rejected_obs) }}" class="form-control @error('rejected_obs') form-control-danger @enderror" > 
...
$(document).ready(function(){
    ...
    $('#formeEdit').submit(function(e){
        e.preventDefault();
        $.ajax({
            url: "{{ route('derive_orders.update', $order->id) }}",
            method: 'PUT',
            data: $('#formeEdit').serialize()+'&_token={{ csrf_token() }}',
