            ...
            <div class="col-lg-8">
                <div class="page-header-title">
                    ...
                    <div class="d-inline">
                        ...
                        <a href="pdf/dependencies" class="btn btn-danger" target="_blank">Listado de Dependencias</a>
                        <a href="/dependencies/exportarexcel" class="btn btn-success">Bajar_en_Excel</a>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <div class="float-left">
                                        <h5>Listado de Dependencias</h5>
                                    </div>
                                    <div class="float-right">
                                        <a href="{{ route('dependencies.create') }}" class="btn btn-primary">Agregar Dependencia</a>  
                                    </div>
                                </div>
                                <div class="card-block">
                                    <div class="dt-responsive table-responsive">
                                        <table id="dependencies" class="table table-striped table-bordered nowrap">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>Dependencias</th>
                                            ...
                                            @for ($i = 0; $i < count($dependencies); $i++)
                                                <tr>
                                                    <td>{{ ($i+1) }}</td>
                                                    <td>{{ $dependencies[$i]->description }}</td>
                                                    <td>
                                                        <button type="button" title="Editar" class="btn btn-warning btn-icon" onclick="updateDependencies({{ $dependencies[$i]->id }})">
                                                            <i class="fa fa-pencil"></i>
                                                        </button>
                                                        <button type="button" title="Borrar" class="btn btn-danger btn-icon" onclick="deleteDependencies({{ $dependencies[$i]->id }})">
