            ...
            <div class="col-lg-8">
                <div class="page-header-title">
                    ...
                    <div class="d-inline">
                        ...
                        <span>Listado de Estados pedidos</span>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                <div class="page-header-breadcrumb">
                    <ul class=" breadcrumb breadcrumb-title">
                        ...
                        <li class="breadcrumb-item">
                            <a href="{{ route('order_states.index') }}">Estados pedidos</a>
                        </li>
                    </ul>
                </div>
            </div>
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <div class="float-left">
                                        <h5>Listado de Estados de Pedidos</h5>
                                    </div>
                                    <div class="float-right">
                                        <a href="{{ route('order_states.create') }}" class="btn btn-primary">Agregar Estado de Pedido</a>  
                                    </div>
                                </div>
                                <div class="card-block">
                                    <div class="dt-responsive table-responsive">
                                        <table id="order_states" class="table table-striped table-bordered nowrap">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                            ...
                                            <tbody>
                                            @for ($i = 0; $i < count($order_states); $i++)
                                                <tr>
                                                    <td>{{ ($i+1) }}</td>
                                                    <td>{{ $order_states[$i]->description }}</td>
                                                    <td>
                                                        <button type="button" title="Editar" class="btn btn-warning btn-icon" onclick="updateOrderState({{ $order_states[$i]->id }})">
                                                            <i class="fa fa-pencil"></i>
                                                        </button>
                                                        <button type="button" title="Borrar" class="btn btn-danger btn-icon" onclick="deleteOrderState({{ $order_states[$i]->id }})">
...
$(document).ready(function(){
    ...
    updateOrderState = function(order_state){
        location.href = '/order_states/'+order_state+'/edit/';
    }

    deleteOrderState = function(order_state){
      swal({
            title: "Atención",
