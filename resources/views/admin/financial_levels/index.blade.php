            ...
            <div class="col-lg-8">
                <div class="page-header-title">
                    ...
                    <div class="d-inline">
                        ...
                        <span>Listado de Tipos de Nivel Financiero</span>
                    </div>
                </div>
            </div>
            <div class="col-lg-4">
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <div class="float-left">
                                        <h5>Listado de Tipos de Nivel Financiero</h5>
                                    </div>
                                    <div class="float-right">
                                        <a href="{{ route('financial_levels.create') }}" class="btn btn-primary">Agregar Tipo de Nivel Financiero</a>  
                                    </div>
                                </div>
                                <div class="card-block">
                                    <div class="dt-responsive table-responsive">
                                        <table id="financial_levels" class="table table-striped table-bordered nowrap">
                                            <thead>
                                                <tr>
                                                    <th>#</th>
                                                    <th>Número</th>
                                                    <th>Tipo de Nivel Financiero</th>
                                                    <th>Acciones</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                            @for ($i = 0; $i < count($financial_levels); $i++)
                                                <tr>
                                                    <td>{{ ($i+1) }}</td>
                                                    <td>{{ $financial_levels[$i]->number }}</td>
                                                    <td>{{ $financial_levels[$i]->description }}</td>
                                                    <td>
                                                        <button type="button" title="Editar" class="btn btn-warning btn-icon" onclick="updateFinancialLevel({{ $financial_levels[$i]->id }})">
                                                            <i class="fa fa-pencil"></i>
                                                        </button>
                                                        <button type="button" title="Borrar" class="btn btn-danger btn-icon" onclick="deleteFinancialLevel({{ $financial_levels[$i]->id }})">
...
$(document).ready(function(){
    $('#financial_levels').DataTable();

    updateFinancialLevel = function(financial_level){
        location.href = '/financial_levels/'+financial_level+'/edit/';
    }

    deleteFinancialLevel = function(financial_level){
      swal({
            title: "Atención",
        ...
        function(isConfirm){
          if(isConfirm){
            $.ajax({
              url : '/financial_levels/'+financial_level,
              method : 'POST',
