...
namespace App\Http\Controllers\MinorPurchase;
...
class MinorPurchasesController extends Controller
{
    /**
    ...
    {
        $this->middleware('checkPermission:minor_purchases.orders.index,admin.orders.index')->only('index'); // Permiso para index 
        $this->middleware('checkPermission:minor_purchases.orders.show,admin.orders.show')->only('show'); // Permiso para show 
    }
    ...
    {
        // obtenemos los pedidos que sean Compras menores y que esén con estado mayor o igual a 20 (PROCESADO PLANIFICACIÓN)
        $orders = Order::whereIn('modality_id', [4,5,6])
        ->where('actual_state', '>=', 20)
        ->get();  
        return view('minor_purchase.minor_purchases.index', compact('orders'));
    }
    ...
    public function show(Request $request, $order_id)
    {
        ...
        $minor_purchase_dependency = $request->user()->dependency_id;
        // Obtenemos los simese cargados por otras dependencias
        $related_simese = $order->simese()->where('dependency_id', '!=', $minor_purchase_dependency)->get();
        // Obtenemos los simese cargados por compras menores
        $related_simese_minor_purchase = $order->simese()->where('dependency_id', $minor_purchase_dependency)->get();
        
        // Obtenemos los archivos cargados por otras dependencias y que no sean de reparo
        $other_files = $order->files()->where('dependency_id', '!=', $minor_purchase_dependency)
                                            ->whereIn('file_type', [0, 3, 4, 5])//0-antecedentes 3-contratos 4-addendas  5-dictamenes 
                                            ->orderBy('created_at','asc')                                            
                                            ->get();
                
        // Obtenemos los archivos cargados por compras menores con tipo de archivos que no sean 1 (reparos dncp)
        $minor_purchase_files = $order->files()->where('dependency_id', $minor_purchase_dependency)->where('file_type', '=', 0)//Archivos normales                                                                                                   
                                                                                                   ->get();

        // Obtenemos los archivos cargados por licitaciones con tipo de archivos 7 cuadro comparativo
        $minor_purchase_files2 = $order->files()->where('dependency_id', $minor_purchase_dependency)->where('file_type', '=', 7)//cuadro comparativo
                                                                                                    ->get();           

        // Obtenemos los archivos cargados por compras menores con tipo de archivos que sean 1 (reparos dncp)
        $minor_purchase_filedncp = $order->files()->where('dependency_id', $minor_purchase_dependency)->where('file_type', '=', 1)->get();
        
        // Obtenemos los archivos cargados por compras menores con tipo de archivos que sean 6 (cosultas dncp)
        $minor_purchase_filedncp_con = $order->files()->where('dependency_id', $minor_purchase_dependency)->where('file_type', '=', 6)->get();
        ...
        // Obtenemos los archivos cargados por compras menores (original)
        //$minor_purchase_files = $order->files()->where('dependency_id', $minor_purchase_dependency)->get();
        

        return view('minor_purchase.minor_purchases.show', compact('order', 'related_simese', 'queries',
                    'related_simese_minor_purchase', 'other_files', 'minor_purchase_files', 
                    'objections','minor_purchase_filedncp','minor_purchase_files2','minor_purchase_filedncp_con'));
    }
    ...
    public function recibeOrder(Request $request, $order_id){
        ...
        if(!$request->user()->hasPermission(['minor_purchases.orders.recibe'])){
            return response()->json(['status' => 'error', 'message' => 'No posee los suficientes permisos para realizar esta acción.'], 200);
        }

        $order = Order::find($order_id);
        // Estado 45 = RECIBIDO COMPRAS MENORES
        $order->actual_state = 45;
        $order->save();
        ...
        $order_order_state->order_state_id = 45;
     ...
     * @return \Illuminate\Http\Response
     */
    public function recibePBCdeUTA(Request $request, $order_id){
        // Chequeamos que el usuario actual disponga de permisos de recibir pedido
        if(!$request->user()->hasPermission(['minor_purchases.orders.recibe'])){
            return response()->json(['status' => 'error', 'message' => 'No posee los suficientes permisos para realizar esta acción.'], 200);
        }
        ...
        $order = Order::find($order_id);
        // Estado 101 = RECIBIDO PBC SIN REPARO DE UTA
        $order->actual_state = 46;
        $order->save();

        // Registramos el movimiento de estado en la tabla orders_order_state
        $order_order_state = new OrderOrderState;
        $order_order_state->order_id = $order->id;
        $order_order_state->order_state_id = 46;
        $order_order_state->creator_user_id = $request->user()->id;
        $order_order_state->save();

        return response()->json(['status' => 'success', 'message' => 'Recibido PBC sin REPARO de UTA'], 200);
    }
    ...
    public function edit(Request $request, $order_id)
    {
        ...
        if($request->user()->hasPermission(['minor_purchases.orders.update'])){
            return view('minor_purchase.orders.update', compact('order'));
        }else{
    ...
    public function update(Request $request, $order_id)
    {
        ...
        return redirect()->route('minor_purchases.show', $order_id)->with('success', 'Llamado modificado correctamente');;
    }
    ...
    public function deriveOrder(Request $request, $order_id){
        ...
        if(!$request->user()->hasPermission(['minor_purchases.orders.derive'])){
            return response()->json(['status' => 'error', 'message' => 'No posee los suficientes permisos para realizar esta acción.'], 200);
        }
        ...
        // PROGRAMAR SIGUIENTE ACCIÓN PARA DERIVAR CUANDO TERMINA EL PROCESO
        // $order = Order::find($order_id);
        // // Estado 50 = PROCESADO COMPRAS MENORES
        // $order->actual_state = 50;
        // $order->save();
     ...
     * @return \Illuminate\Http\Response
     */
    public function deriveUTA(Request $request, $order_id){
        // Chequeamos que el usuario actual disponga de permisos de derivacion de pedido
        if(!$request->user()->hasPermission(['minor_purchases.orders.derive'])){
            return response()->json(['status' => 'error', 'message' => 'No posee los suficientes permisos para realizar esta acción.'], 200);
        }
        ...
        $order = Order::find($order_id);
        // Estado 141 = PROCESADO INFORME EVAL POR COMPRAS MENORES
        $order->actual_state =  141;
        $order->save();

        // Registramos el movimiento de estado en la tabla orders_order_state
        $order_order_state = new OrderOrderState;
        $order_order_state->order_id = $order->id;
        $order_order_state->order_state_id = 141;
        $order_order_state->creator_user_id = $request->user()->id;
        $order_order_state->save();

        return response()->json(['status' => 'success', 'message' => 'Informe de Evaluación remitido a UTA'], 200);
    }    
     ...
     * @return \Illuminate\Http\Response
     */
    public function deriveAJ_NoCovid(Request $request, $order_id){
        // Chequeamos que el usuario actual disponga de permisos de derivacion de pedido
        if(!$request->user()->hasPermission(['minor_purchases.orders.derive'])){
            return response()->json(['status' => 'error', 'message' => 'No posee los suficientes permisos para realizar esta acción.'], 200);
        }
        ...
        $order = Order::find($order_id);
        // Estado 142 = DERVADO A AJ PARA DICTAMEN
        $order->actual_state =  142;
        $order->save();

        // Registramos el movimiento de estado en la tabla orders_order_state
        $order_order_state = new OrderOrderState;
        $order_order_state->order_id = $order->id;
        $order_order_state->order_state_id = 142;
        $order_order_state->creator_user_id = $request->user()->id;
        $order_order_state->save();

        return response()->json(['status' => 'success', 'message' => 'Informe de Eval. y antecedentes remitidos a AJ para Dictamen'], 200);
    }    
    ...
    {
        // obtenemos los pedidos que sean de Compras Menores modality = 4, 5 o 6
