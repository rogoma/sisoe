                                ...
                                <div class="card-header">
                                    <div class="row">
                                        ...
                                        <div class="col-sm-2">
                                            ...
                                            @if (Auth::user()->hasPermission(['documentals.orders.derive']) && in_array($order->actual_state, [35]))
                                            ...
                                            @else
                                                @if (Auth::user()->hasPermission(['documentals.orders.derive']) && in_array($order->actual_state, [100]))
                                                    ...
                                                    <a style="font-size: 14px; font-weight: bold; color:blue;background-color:lightblue;" class="dropdown-item waves-effect f-w-600; text-danger" href="javascript::void(0);" onclick="recibeOrderReparo({{ $order->id }});">Recibir Eval. c/reparo de UTA</a>
                                                @else
                                                    <button class="btn btn-primary dropdown-toggle waves-effect" type="button" id="acciones" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">Acciones</button>
                                                @endif
                                            @endif
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="acciones" data-dropdown-in="fadeIn" data-dropdown-out="fadeOut">
                                                ...
                                                @if (Auth::user()->hasPermission(['documentals.orders.derive']) && in_array($order->actual_state, [135,136]))
                                                    <a class="dropdown-item waves-effect f-w-600" href="javascript::void(0);" onclick="deriveInforme({{ $order->id }});">Derivar Informe de Evaluación a UTA</a>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="tab-content card-block">
                                        ...
                                        <div class="tab-pane" id="tab7" role="tabpanel">
                                            ...
                                            <div class="text-right">
                                                ...
                                                @if(count($related_simese_user) == 0 && Auth::user()->hasPermission(['admin.simese.create', 'orders.simese.create', 'documentals.simese.create']))
                                                <a href="{{ route('orders.simese.create', $order->id) }}" class="btn btn-primary">Cargar N° SIMESE</a>
                                                @elseif(Auth::user()->hasPermission(['admin.simese.update', 'orders.simese.update', 'documentals.simese.update']))
                                        ...
                                        <div class="tab-pane" id="tab8" role="tabpanel">
                                            ...
                                            <div class="text-right">
                                                @if (Auth::user()->hasPermission(['admin.files.create', 'orders.files.create', 'documentals.files.create','utas.files.create']))
    ...
    recibeOrder = function(order_id){
      swal({
        ...
        function(isConfirm){
          if(isConfirm){
            $.ajax({
                url : '/documentals/recibeOrder/'+order_id,
                method: 'POST',
    ...
    recibeOrderReparo = function(order_id){
      swal({
        ...
        function(isConfirm){
          if(isConfirm){
            $.ajax({
                url : '/documentals/recibeOrderReparo/'+order_id,
                method: 'POST',
                data: '_token='+'{{ csrf_token() }}',
                success: function(data){
                    try{
    ...
    deriveInforme = function(order_id){
      swal({
        ...
        function(isConfirm){
          if(isConfirm){
            $.ajax({
                url : '/documentals/deriveInforme/'+order_id,
                method: 'POST',
                data: '_token='+'{{ csrf_token() }}',
                success: function(data){
                    try{
